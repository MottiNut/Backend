...
public class AuthBffService {
    ...
    public AuthResponse registerPatient(RegisterPatientRequest request, MultipartFile profileImage) {
        try {
            byte[] profileImageBytes = profileImage != null ? profileImage.getBytes() : null;
            String profileImageContentType = profileImage != null ? profileImage.getContentType() : null;
            ...
            Patient patient = authService.registerPatient(
                    ...
                    null, // emergencyContact - se puede actualizar después
                    request.getGender(),
                    profileImageBytes,
            ...
            return AuthResponse.builder()
                    ...
                    .fullyVerified(patient.isFullyVerified())
                    .requiresVerification(true)
                    .message("Registro exitoso. Te hemos enviado un código de verificación a tu email.")
                    .build();
        } catch (IOException e) {
            throw new com.mottinut.shared.domain.exceptions.ValidationException("Error al procesar la imagen de perfil");
        }
    }
